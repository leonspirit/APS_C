    ...
    GetDetailPembelian(currentToken, id, function(result){
       ...
       // var  CicilanPenjualanTable;// =$("#Detailpenjualan-CicilanTable").DataTable();
        if (result.token_status=="success") {
            ...
            var subtotal = parseInt(Math.round(pembelian.subtotal *(100-pembelian.disc)/100));
            ...
            var grandTotalText = "<span class='pull-right'>Rp." + numberWithCommas(subtotal) + "</span>";

            $(itemPenjualanBesarTable.column(8).footer()).html(grandTotalText);
            $(itemPenjualanBesarTable.column(6).footer()).html("<span class='pull-right'>"+pembelian.disc+"%</span>");
...
function PopulatePembelianKecilInvoice(id)
{
    var currentToken = localStorage.getItem("token");
    GetDetailPembelian(currentToken, id, function(result){
        var  itemPenjualanBesarTable;// =$("#Detailpenjualan-ItemTable").DataTable();
        ...
        if (result.token_status=="success") {
            ...
            if (!$.fn.DataTable.isDataTable("#PembelianKecil-ItemTable")) {
                itemPenjualanBesarTable = $("#PembelianKecil-ItemTable").DataTable({
                    "paging": false,
                    "lengthChange": false,
                    "searching": false,
                    "ordering": false,
            ...
            document.getElementById("PembelianKecil-PelangganText").innerHTML = pembelian.supplierNama;
            document.getElementById("PembelianKecil-TglJatuhTempoText").innerHTML = JatuhTempoText;
            document.getElementById("PembelianKecil-TglTransaksiText").innerHTML = TglTransaksiText;
            document.getElementById("PembelianKecil-PembayaranText").innerHTML = PembayaranText;
           document.getElementById("PembelianKecil-KodeText").innerHTML = StrId;

            var grandTotalText = "<span class='pull-right'>Rp." + numberWithCommas(subtotal) + "</span>";

            $(itemPenjualanBesarTable.column(3).footer()).html(grandTotalText);
            $(itemPenjualanBesarTable.column(1).footer()).html("<span class='pull-right'>"+pembelian.disc+" %</span>");
            console.log(pembelian);
            for (i = 0; i < pembelian.barang.length; i++) {
                var hargaUnit = pembelian.barang[i].harga_per_biji;
                var qty = pembelian.barang[i].quantity;
